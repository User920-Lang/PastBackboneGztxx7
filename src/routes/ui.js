...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    #liveTag{font-size:0.75rem;color:#4caf50;margin-left:8px}
  </style>
</head>
<body>
<header>
  ...
  <span id="liveTag"><span class="dot dot-live"></span>ao vivo</span>
</header>
<main>
  ...
  <div id="tournaments" class="section active">
    <div class="card">
      ...
      <table>
        <thead>
          <tr><th>ID</th><th>Nome</th><th>Descrição</th><th>Cor</th><th>Data</th><th>Hora</th><th>Status</th><th>Jogadores</th><th>Ação</th></tr>
        </thead>
...
<script>
  ...
  function renderTable() {
    ...
    if (!list.length) { tbody.innerHTML = '<tr><td colspan="9" class="empty">Nenhum torneio</td></tr>'; return; }
  ...
  document.getElementById('configForm').addEventListener('submit', async e => {
    ...
    try {
      ...
      if (res.ok) { msg.className='msg ok'; msg.textContent='Salvo com sucesso!'; }
  ...
  document.getElementById('createForm').addEventListener('submit', async e => {
    ...
    try {
      ...
      if (res.ok) {
        msg.className='msg ok'; msg.textContent='Torneio criado!';
        e.target.reset();
        fetchTournaments();
      } else throw new Error();
